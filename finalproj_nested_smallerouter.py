...
tri.add_region((1,29), 0, maximum_area= 500) #active domain
...
dis = flopy.mf6.ModflowGwfdisv(gwf, length_units='FEET', nlay=nlay, 
                               ncpl=ncpl, 
...
ic = flopy.mf6.ModflowGwfic(gwf, strt=20)
...
#create the plot (Based on Tutorial from Hatari Labs)
...
ax = plt.subplot(1, 1, 1, aspect='equal')
img=tri.plot(ax=ax, edgecolor='lightsteelblue',a=head[0, 0, :], 
             cmap='RdBu', vmax=50, vmin=-100, alpha=0.8)
...
ax.set_xlabel('East to West (200m Intervals)', fontsize='20')
ax.set_ylabel('South to North (200m Intervals)', fontsize='20')
ax.set_title('Triangular Mesh Domain: 2 Wells (Nested Regions/Finer Outer Domain)', fontsize='20')
colorbar.set_label('Head Elevations (m)', fontsize='15')

endtime = time.time()
cpuend= time.process_time()

plt.savefig('/home/jrevier/Documents/ahs432/Final_Project/Plots/nested_smallerouter_example', 
            format='png')
